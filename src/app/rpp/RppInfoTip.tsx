...
const HELP = {
  ...
  "運用モード": "通常運用のROAS・検索順位・バランス・CPC固定の4モードを示します。",
  "モード・期間": "実験で固定している運用モードと開始・終了期間です。",
  "保護": "ブロック・ホワイト・変更不可・注力など、自動提案を制御する安全区分です。",
  "配信": "商品広告の現在の除外状態です。除外操作は商品CPC行だけで行います。",
  "夜間停止": "商品単位の夜間停止です。ONにすると毎日01:30にRPP広告をOFF、06:00にONへ戻す対象になります。キーワードCPC単位では設定できません。",
  "操作": "設定、CSV出力、除外・再開など、この行に対して実行できる操作です。",
  "ブロック理由": "安全ルールにより自動提案の対象外になった理由です。",
  ...
  "商品CPC上限": "商品別上限が未設定の場合に使う、商品CPCの標準上限です。商品別設定が優先されます。",
  "KW CPC上限": "商品別上限が未設定の場合に使う、キーワードCPCの標準上限です。商品別設定が優先されます。",
  "1回最大上げ": "各判定1回で現在CPCから上げられる最大金額です。条件を満たせば同じ日に再調整します。",
  "1回最大下げ": "各判定1回で現在CPCから下げられる最大金額です。条件を満たせば同じ日に再調整します。",
  ...
  "RPP設定KW": "RPPへ実際に設定するキーワードです。基準ワードとは別に管理します。",
  "基準ワード": "商品CPCの検索順位を判定するワードです。複数追加でき、どれか1つでもPC・SPの目標順位を満たせば達成扱いです。",
  ...
  "最適化モード": "ROASモード・検索順位モード・バランスモード・CPC固定モードの4つから選択します。",
  "固定CPC": "CPC固定モードで維持する入札単価です。RMSへ直接登録するCPCと同じ考え方です。",
  "CPC上限": "この対象で提案できるCPCの個別上限です。",
  "旧CPC上限": "既存の固定CPC実験または旧データに保存されているCPC上限です。",
  "モード別CPC下限": "選択中の通常運用モード専用のCPC下限です。商品20円・キーワード40円の楽天下限が常に優先されます。",
  "モード別CPC上限": "選択中の通常運用モード専用のCPC上限です。未設定の場合は既存データの旧CPC上限を引き継ぎます。",
  ...
  "選択モードの自動調整を有効化": "ROAS・検索順位・バランスを選択した設定行の自動調整を有効にする全体スイッチです。CPC固定は対象外です。",
