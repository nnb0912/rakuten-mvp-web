...
test("画面の見方から使い方マニュアル動画を再生できる", () => {
  const page = readFileSync(join(rppDir, "page.tsx"), "utf8");
  const css = readFileSync(join(process.cwd(), "src", "app", "globals.css"), "utf8");
  const video = join(process.cwd(), "public", "rpp", "manuals", "rpp-control-guide-v13.mp4");
  assert.match(page, /<video[^>]*controls[^>]*playsInline/s);
  assert.match(page, /src="\/rpp\/manuals\/rpp-control-guide-v13\.mp4"/);
  assert.match(page, /5分04秒/);
