...
- WALは `PREPARED → SUBMITTING → SUBMITTED → VERIFIED`。復旧時に所有権を確定できるのは `VERIFIED` のみ。
...
```bash
python3 -m unittest -v test_rpp_product_delivery_scheduler.py test_rpp_product_night_pause.py
